...
QList<QWidget*> ScreenGrabber::createMonitorPreviews(const QPixmap& fullScreenshot,
                                                     QScreen* cursorScreen)
{
    ...
    for (int screenIndex = 0; screenIndex < screens.size(); ++screenIndex) {
        ...
        if (wayland) {
            ...
            picker->adjustSize();
            picker->move(screenGeom.x() + (screenGeom.width() -
                                             picker->width()) /
                                           2,
                         screenGeom.y() + screenGeom.height() -
                           picker->height() - 16);
        }
